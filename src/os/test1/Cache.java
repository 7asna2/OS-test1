...
public class Cache {
    ...
    public void readRequest(Process newProcess) { //reading request from cash  
       ...
       // if (!tryHit(newProcess)) {
            ...
            if (!addProcess(newProcess)) {//if there is no place in cache try to replace
                //out tem should return it back to memory
    ...
    boolean tryHit(Process newProcess) {   //try to git process from the cache
        for (int i = 0; i < size; i++) {
            if (newProcess == caschedProcess[i]) {
                ...
                Process p = mm.SearchingForProcess(newProcess);
    ...
    private boolean addProcess(Process newProcess) {
        ...
        return false;       //use one of the replacement algorithms.
        //finally
    }
